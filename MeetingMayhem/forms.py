...
class MessageForm(FlaskForm):
	...
	def validate_encryption_and_signed_keys(self, encryption_and_signed_keys):
		...
		if encryption_and_signed_keys.data == '':
			return
		for element in keys:
			if (not(bool(re.match("signed[(][a-zA-Z0-9]+[.](private|public)[)]$", element))) and not(bool(re.match("symmetric[(][a-zA-Z0-9]+[.](private|public)[)]$", element))) and not(bool(re.match("asymmetric[(][a-zA-Z0-9]+[.](private|public)[)]$", element)))):

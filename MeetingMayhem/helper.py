...
def create_message(user, game, request, form):
    ...
    if user.role == 3: #if the user is an adversary
        ...
        #create the message and add it to the db
        new_message = Message(round=game.current_round+1, game=game.id, sender=user.username, recipient=recipients, content=form.content.data, is_edited=False, new_sender=None, new_recipient=None, edited_content=None, is_deleted=False, adv_created=False, is_encrypted=(len(encrypted_keys)> 0), recipient_encryption_details = encrypted_keys_string, is_signed= (len(signed_keys) > 0), sender_encryption_details = signed_keys_string)
        db.session.add(new_message)
        db.session.commit()
        #display success to user
        flash(f'Your message has been sent!', 'success')

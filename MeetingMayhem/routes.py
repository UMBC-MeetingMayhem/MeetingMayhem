...
def spectate_game():
	...
	if current_user.role != 5:
		...
		if game_selected and select_game_form.validate():
			...
			message = Message.query.filter_by(game=game.id).all()
			return render_template('spectator_messages.html', title='Spectate Game Info', game=game, message=message)

		# Send list of games to template
		return render_template('spectator_messages.html', title='Spectate A Game', sg_form=select_game_form, game=None)
